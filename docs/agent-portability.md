...
`skills/` and `scripts/`; host-specific files are thin adapters that
...
| Host | Files | Notes |
| --- | --- | --- |
| Claude Code | `.claude-plugin/`, `.claude-plugin/hooks.json`, `skills/` | Full plugin marketplace install with the `chat2skill` skill, `UserPromptSubmit`, Stop learning, and Stop response guard hooks. |
| Codex | `.codex-plugin/plugin.json`, `.codex-plugin/hooks.json`, `skills/` | Plugin install with Codex-specific lifecycle hooks, including Stop learning and Stop response guard. `install.sh` refreshes local cache copies during development. |
...
existing `skills/` and `scripts/` files. When a host only supports
